...
- **Vendor ESPAsyncWebServer with a send-loss patch; pin AsyncTCP** ([`lib/ESPAsyncWebServer/`](lib/ESPAsyncWebServer/), [`platformio.ini`](platformio.ini)): The web server now lives in-repo (me-no-dev `ad3741d` = **v3.6.0**, the baseline this device ran stably for months) with a local `_ack` patch, and `AsyncTCP` pins **`ESP32Async/AsyncTCP @ 3.3.8`** instead of resolving unpinned. The patch fixes the root cause of portal corruption: upstream `AsyncAbstractResponse::_ack` ignored short/zero `AsyncClient::write()` results (lwIP `ERR_MEM` under heap/pbuf pressure), silently dropping one fill-sized block mid-response (pages arriving 2 KiB short / rendering raw CSS) while inflating `_in_flight` with phantom bytes (transfers stalling until browser timeout, then `503 portal page busy` from the gate). The patch keeps refused wire bytes in a carry buffer and resends them on later acks before any new fill, so the stream stays byte-identical. **Do not upgrade to ESP32Async ESPAsyncWebServer 3.9+/AsyncTCP 3.4/3.5 on the Arduino 2.x core**: bench testing on the Atom Lite reproduced (100%, within 4 request rounds) a full lwIP freeze — device off the network, no ping, loop() still running, no watchdog reset — whenever a client aborted an in-flight transfer; their synchronization assumes lwIP core locking that Arduino 2.x does not enable.

### Fixed

- **"portal page assembly failed (low memory)" root-caused; now auto-retries** ([`lib/spaWebServer/spaWebServer.cpp`](lib/spaWebServer/spaWebServer.cpp)): On-device heap forensics (`heap_caps_get_info(MALLOC_CAP_8BIT)` + integrity check at the failure moment) proved the error is **genuine DRAM exhaustion hidden by a lying metric**: `ESP.getFreeHeap()`/`getMaxAllocHeap()` include a ~40 KB 32-bit-only IRAM region plain `malloc` can never use (the eerily constant `maxAlloc=42996`). Real usable DRAM at failure: 7.6 KB free, 2.4 KB largest block — the 73 KB `/status` body plus the SPA's parallel JSON responses do not fit together in the PICO-D4's ~87 KB of usable DRAM. Mitigations: slab allocation retries with yields, portal memory guards now read `MALLOC_CAP_8BIT` (honest numbers), and the dead-end plain-text 503 is replaced by a tiny PROGMEM page that auto-refreshes after 2 s (plus `Retry-After`), by which time hydration traffic has drained — bench storm test: 9/12 heavy-overlap rounds delivered the full page within one retry. **The structural fix is the planned static-shell + JSON-hydration portal rewrite, which eliminates the 73 KB RAM body entirely.**
- **Device dropped off the network mid-transfer — now self-heals in ~15 s** ([`lib/spaWebServer/spaWebServer.cpp`](lib/spaWebServer/spaWebServer.cpp)): Bench root-caused with in-firmware task-state probes: under sustained large HTTP transfers, the **Wi-Fi driver's RX path dies silently** on the Arduino 2.x core (espressif32@6.8.1 / arduino 2.0.17). Post-mortem while frozen: no ping/HTTP from outside, `WiFi.status()` still `WL_CONNECTED`, all network tasks Blocked-normal, heap healthy, and lwIP still answers `tcpip_try_callback` echoes — which is why the loopback HTTP liveness probe (2.26.2) could never detect it: loopback traffic never crosses the radio. Two watchdogs added, both silent unless failing: an **lwIP heartbeat** (non-blocking `tcpip_try_callback` echo every 10 s; two misses → restart — catches true tcpip wedges) and a **Wi-Fi path watchdog** (async `esp_ping` to the gateway every 10 s, armed only after the first success per connection so ICMP-dropping routers cannot cause restart loops; three consecutive misses while associated → restart, reason `Wi-Fi path watchdog`). Verified on the bench: deliberate freeze → detected → self-restart → serving again in **14 s**. `-DSPA_NET_TASK_PROBE` re-enables the 15 s task-state serial dump used for diagnosis. The freeze reproduced on every AsyncTCP/ESPAsyncWebServer combination tried (3.3.8/3.6.0 through 3.5.0/3.12.0), so the true fix is a future migration to the Arduino 3.x core (newer Wi-Fi driver); until then the watchdogs cap the outage.
- **AsyncTCP event-queue deadlock hardening** ([`lib/AsyncTCP/`](lib/AsyncTCP/), [`platformio.ini`](platformio.ini)): AsyncTCP is vendored (ESP32Async v3.3.8) with defensive fixes found during the same investigation: event posts from lwIP's tcpip task no longer wait forever on a full queue (bounded 100 ms wait, then drop + `ERR_TIMEOUT` so lwIP re-delivers refused recv data), `_tcp_clear_events` no longer blocks the queue's own consumer task (guaranteed self-deadlock on a full queue), and the queue is 256 slots via `CONFIG_ASYNC_TCP_QUEUE_SIZE`. These close real AB-BA deadlock paths even though the bench freeze above turned out to live in the Wi-Fi driver layer.
- **HTTP liveness watchdog could reboot mid-page** ([`lib/spaWebServer/spaWebServer.cpp`](lib/spaWebServer/spaWebServer.cpp)): Liveness probing now defers while a large portal page is in flight (`s_portalLargePageInFlight`), so a slow or stalled `/status`-class transfer can no longer walk the probe fail streak up to an `ESP.restart()` while a client is downloading.
...
- Firmware **`VERSION`** is **`2.27.0`** ([`src/main.h`](src/main.h)); **`ANALYTICS_VERSION`** aligned ([`lib/Analytics/Analytics.h`](lib/Analytics/Analytics.h)).
