...
**PR compile CI:** [`.github/workflows/build.yml`](.github/workflows/build.yml) runs `pio run` for **`M5AtomLite-tub`** and **`ESP32ota`** on pull requests and pushes to **`ESP32`**. The job copies **`src/config-example.h`** → **`src/config.h`** on the runner only (never commit private `config.h`). A [branch ruleset](https://github.com/shomanjk/esp32_balboa_spa/rules/19921421) requires those two checks before merging into **`ESP32`** (admins can bypass; direct pushes for tiny docs still work). Wait for Build plus **automatic Codex** (on PR open). Comment `@codex review` only if auto review never appears, after substantive follow-up commits, or for a focused ask — not by default on open, and not after a clean pass for tiny docs tweaks. Wiki publish ([`publish-wiki.yml`](.github/workflows/publish-wiki.yml)) is separate and is not a PR gate.
