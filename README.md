...
**Alternate wiring:** **Tail485** ([tail stack](https://docs.m5stack.com/en/atom/tail485)) and **Unit RS485** ([Grove](https://docs.m5stack.com/en/unit/rs485)) on Atom Lite use **`M5AtomLite-tub`** with the **`#undef` / 32/26 block** in [`src/config-example.h`](src/config-example.h) (overrides env **22/19**). **Generic `ESP32ota`** is for **non-M5 ESP32 dev boards** (`board = esp32dev`; first USB flash: `--upload-protocol esptool`). Atom Lite MCU is **ESP32-PICO-D4**. Use **`AUTO_TX true`**; separate **DE/RE GPIO from the Atom is not supported**. See wiki **[alternate RS485 (32/26 pins)](https://github.com/shomanjk/esp32_balboa_spa/wiki/Hardware-targets#atom-lite--alternate-rs485-3226-pins)**.
