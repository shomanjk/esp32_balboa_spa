...
**Alternate wiring:** On a **generic** env, set pins in `config.h` (defaults **16/17**, or **32/26** for [Tail485](https://docs.m5stack.com/en/atom/tail485) / [Unit RS485](https://docs.m5stack.com/en/unit/rs485) — see [`src/config-example.h`](src/config-example.h)). Do not expect `config.h` pin overrides to win on `M5*-tub` envs. The Atom Lite MCU is **ESP32-PICO-D4**. **Tail485** is a **tail stack** (like Atomic base, not Grove); **Unit RS485** uses the **Grove** port — both use **32/26** UART pins and need a **generic** env, not `M5AtomLite-tub` (**22/19**). Use **`AUTO_TX true`**. Separate **DE/RE GPIO from the Atom is not supported**. See wiki **[Hardware targets — alternate RS485 (32/26 pins)](https://github.com/shomanjk/esp32_balboa_spa/wiki/Hardware-targets#atom-lite--alternate-rs485-3226-pins)**.
