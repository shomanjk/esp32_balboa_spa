...
| *(not reported)* | [M5 Atom Lite](https://docs.m5stack.com/en/core/ATOM%20Lite) (ESP32-PICO-D4) + **Tail485** (Grove) | Generic env + `config.h` (not `M5AtomLite-tub`) | **`TX485_Rx` 32**, **`TX485_Tx` 26**; **`AUTO_TX true`**; reporter also tried **`RS485_DIR_PIN`** (unsupported — see [Hardware targets — Grove alternate](Hardware-targets#atom-lite--grove-rs485-alternate)) | **Unverified on v2.28+** — [Issue #31](https://github.com/shomanjk/esp32_balboa_spa/issues/31): RS485 worked initially; web portal hung/crashed (likely pre-**2.28** portal bugs on PICO-D4). Reporter switched to another firmware before retesting current releases. Same MCU as Atomic-base stack — different transceiver and env/pin setup. |
