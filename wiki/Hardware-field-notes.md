...
| *(not reported)* | [M5 Atom Lite](https://docs.m5stack.com/en/core/ATOM%20Lite) (ESP32-PICO-D4) + [**Tail485**](https://docs.m5stack.com/en/atom/tail485) (tail stack — **not** Grove) | Generic env + `config.h` (not `M5AtomLite-tub`) | **`TX485_Rx` 32**, **`TX485_Tx` 26**; **`AUTO_TX true`**; reporter also tried **`RS485_DIR_PIN`** (unsupported — see [Hardware targets — alternate 32/26](Hardware-targets#atom-lite--alternate-rs485-3226-pins)) | **Unverified on v2.28+** — [Issue #31](https://github.com/shomanjk/esp32_balboa_spa/issues/31): RS485 worked initially; web portal hung/crashed (likely pre-**2.28** portal bugs on PICO-D4). Reporter switched to another firmware before retesting current releases. Same MCU as Atomic-base stack — different transceiver, **32/26** pins, generic env. |
