...
This page lists **supported** boards (matching a checked-in PlatformIO **`[env:…]`**) and **planned / community bring-up** targets. It is **not** a substitute for release notes: shipped behavior stays documented in the [README](https://github.com/shomanjk/esp32_balboa_spa/blob/main/README.md) and [CHANGELOG](https://github.com/shomanjk/esp32_balboa_spa/blob/main/CHANGELOG.md).
...
| **M5 Atom Lite** + Atomic RS485 Base | ESP32-PICO-D4 · 4 MB | `M5AtomLite-tub`, `M5AtomLite-tub-ota` | **Supported** ([README — M5 section](https://github.com/shomanjk/esp32_balboa_spa/blob/main/README.md#m5-atom-lite--atomic-rs485-base-tub-side)) |
...
**Tub-side compile-time feature flags** used by shipped gateway envs mirror [`platformio.ini`](https://github.com/shomanjk/esp32_balboa_spa/blob/main/platformio.ini): typically **`LOCAL_CLIENT`**, **`LOCAL_CONNECT`**, and **`BRIDGE`** (plus shared **`PRODUCTION`** / logging). The RS485 framing and MQTT stack do not depend on which ESP32 variant you pick; **pins, bootloader, partitions, and M5 libs** do.
...
1. **RS485 UART GPIOs:** The supported **Atom Lite on [Atomic RS485 Base](https://docs.m5stack.com/en/atom/Atomic%20RS485%20Base)** pairing uses firmware **`TX485_Rx` 22 / `TX485_Tx` 19** (see [`src/config-example.h`](https://github.com/shomanjk/esp32_balboa_spa/blob/main/src/config-example.h)). **Do not** copy those pins to AtomS3 until M5 documentation (or your own probing) confirms the **same stack** routes UART to the same lines for the S3 module — HY2.0/header pins differ between products.
2. **`config-example.h`:** Avoid adding unverified pin blocks there; misleading comments are worse than “TBD on wiki.”
3. **Optional status LED:** Today’s tub-side RGB path is **`M5_ATOM_LED`** with [`m5stack/M5Atom`](https://registry.platformio.org/libraries/m5stack/M5Atom) and [`src/led_control.cpp`](https://github.com/shomanjk/esp32_balboa_spa/blob/main/src/led_control.cpp). AtomS3-class boards are typically brought up with **M5Unified** in M5’s own PlatformIO examples; porting or gating LED code is **deferred** until a maintainer-owned device is on the bench.

### Partition / flash size

[`spa_module.csv`](https://github.com/shomanjk/esp32_balboa_spa/blob/main/spa_module.csv) is **4 MB–oriented** (fits Atom Lite). An **8 MB** AtomS3 Lite can usually use the **same** CSV (unused flash at the end). Resizing app or LittleFS is **optional** future work if you need more on-chip space.
...
4. Update [README](https://github.com/shomanjk/esp32_balboa_spa/blob/main/README.md), [`AGENTS.md`](https://github.com/shomanjk/esp32_balboa_spa/blob/main/AGENTS.md), and **[CHANGELOG](https://github.com/shomanjk/esp32_balboa_spa/blob/main/CHANGELOG.md)** when an env merges.

---

## Wiki publishing

Repo copies under **`wiki/`** sync to GitHub Wiki via CI (see [`wiki/BOOTSTRAP.md`](https://github.com/shomanjk/esp32_balboa_spa/blob/main/wiki/BOOTSTRAP.md) and `.github/workflows/publish-wiki.yml`).
