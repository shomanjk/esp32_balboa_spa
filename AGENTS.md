...
| **M5 Atom Lite + Atomic RS485 Base** | **`M5AtomLite-tub`**, **`M5AtomLite-tub-ota`** | `board = m5stack-atom`. Env **`build_flags`** set **RX 22** / **TX 19** / **`AUTO_TX`** (omit pin overrides in `config.h`). **`M5_STATUS_LED`** FastLED GPIO **27** — [README](README.md). Nonstandard Grove wiring: use a **generic** env + `config.h` pins. |
