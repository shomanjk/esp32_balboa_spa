...
| **M5 Atom Lite + Atomic RS485 Base** | **`M5AtomLite-tub`**, **`M5AtomLite-tub-ota`** | `board = m5stack-atom` (**ESP32-PICO-D4**). Env **`build_flags`** set **RX 22** / **TX 19** / **`AUTO_TX`** (omit pin overrides in `config.h`). **`M5_STATUS_LED`** FastLED GPIO **27** — [README](README.md). **Grove RS485** (Unit RS485, Tail485): **generic** env + **`config.h`** pins **32/26** — not `M5AtomLite-tub`. Manual DE/RE GPIO / **`RS485_DIR_PIN`** not supported. Wiki: [Hardware targets — Grove alternate](https://github.com/shomanjk/esp32_balboa_spa/wiki/Hardware-targets#atom-lite--grove-rs485-alternate). |
