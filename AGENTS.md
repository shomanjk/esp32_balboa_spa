...
- **`src/config.h`** is **gitignored** (see `.gitignore`). Copy from [`src/config-example.h`](src/config-example.h) and set Wi‑Fi / MQTT. **`TX485_Rx` / `TX485_Tx` / `AUTO_TX`:** set in `config.h` for **generic** envs; **M5\*‑tub** envs supply them via `platformio.ini` `build_flags` (use `#ifndef` guards — see example). Optional: **`AUTO_SYNC_PANEL_CLOCK`** (panel time from NTP once per boot when drift exceeds threshold; example defaults **on**, omitted in older configs → **off** — see [`spaCommandDispatcher.h`](lib/spaMessage/spaCommandDispatcher.h)). Optional MQTT overrides: **`MQTT_HA_DISCOVERY`**, **`MQTT_DISCOVERY_PREFIX`**, **`MQTT_HA_TEMP_UNIT`** (see [`lib/mqttModule/mqttModule.h`](lib/mqttModule/mqttModule.h) defaults). Optional **HTTP liveness watchdog:** **`HTTP_LIVENESS_WATCHDOG`**, **`HTTP_LIVENESS_PROBE_INTERVAL_MS`**, **`HTTP_LIVENESS_PROBE_TIMEOUT_MS`**, **`HTTP_LIVENESS_PROBE_FAIL_MAX`**, **`HTTP_LIVENESS_MIN_UPTIME_MS`** (loopback **GET `/api/version`**; restart after consecutive failures — omitted → on with defaults in example).
...
| Area | Location | Notes |
|------|----------|--------|
| RS485 framing, CTS, CRC, spa **ID** (`0x0A`) | [`lib/localRS485Communication/rs485.cpp`](lib/localRS485Communication/rs485.cpp) | Inbound → `spaReadQueue`; outbound from `spaWriteQueue` on Clear-to-Send. **`rs485Setup()`** only initializes stats / boot-safety RTC; **`Serial2.begin` is deferred** via **`rs485EnsureUartBegun()`** until Wi‑Fi is up and **`ArduinoOTA.begin()`** has run (`wifiOtaIsStarted()`). **Safe mode** (skip UART) after Atom Lite GPIO **16/17** pin guard or repeated post-begin WDT/panic boots; clear with **`POST /api/rs485/retry`** or power-on. Health codes include **`UART_DEFERRED`** / **`RS485_SAFE_MODE`**. |
| Message parse, status, config requests | [`lib/spaMessage/spaMessage.cpp`](lib/spaMessage/spaMessage.cpp), [`lib/spaMessage/balboa.h`](lib/spaMessage/balboa.h) | `sendMessageToSpa()` enqueues to **`spaWriteQueue`** (returns **`false`** when UART deferred/safe or queue full). Config polling skips while UART not begun. |
...
| Web / SCI emulation | [`lib/spaWebServer/spaWebServer.cpp`](lib/spaWebServer/spaWebServer.cpp) | **`GET /`** serves **Spa Status** (same handler as **`/status`**); **`/state`** is the ESP State page (**Check for updates** compares gateway **`VERSION`** to latest GitHub release and **`src/main.h`** on **`FIRMWARE_REPO_DEFAULT_BRANCH`** via browser → GitHub API; links to Releases and **Source (branch)** as tagged vs tip source snapshots). **`parseBody`**: reads work; **`device_request`** dispatches **`Button`**, **`SetTemp`**, **`SystemTime`**, **`TempUnits`**, **`TimeFormat`**, and **`Filters`** (read via **`Request`/`Filters`**, write via base64 blob → **`spaSetFilterCycles`**) to [`spaCommandDispatcher`](lib/spaMessage/spaCommandDispatcher.cpp) (Balboa **`0x11`** / **`0x20`** / **`0x21`** / **`0x23`** / **`0x27`**) with accepted/rejected XML. Multi-chunk POST body handling is implemented. When no spa data exists yet, `/devices/sci` returns explicit not-ready XML (`ready=false`, `error=no_spa_data_yet`) instead of 404. **`GET /api/status/controls`** feeds live **`/status`** polling (includes **`reminderText`**, **`reminderHint`**). **`/config`** shows decoded fault-log fields when available. JSON: **`GET /api/version`**, **`GET /api/diagnostics`** (includes **`otaRunningPartition`**, **`otaPartitionState`**, **`otaBootVerified`** when rollback is enabled), **`GET /api/wifi`**, **`GET/POST /api/config/filter`**, **`GET /api/config/export`**, **`POST /api/config/import`**, **`GET /api/rs485`**, **`POST /api/rs485/retry`**. **Logs:** **`GET /logs`**, **`GET /api/logs?since=&limit=`**, **`WebSocket /api/logs/ws`**, **`GET/POST /api/logs/config`**; capture in [`lib/webLogBuffer/`](lib/webLogBuffer/) (tee on `Serial`; **recursive mutex** serializes tee assembly + ring + Serial so **`Log` from AsyncTCP** (e.g. bridge) does not race **`loop`**). **`/config`**: editable filter schedules + backup/restore toolbar ([`spaConfigExport.cpp`](lib/spaWebServer/spaConfigExport.cpp)). **Portal page gate:** only one of **`/status`**, **`/config`**, **`/state`**, **`/logs`** assembles/sends a large HTML body at a time (503 `portal page busy`; release on client disconnect). **HTTP liveness watchdog:** **`spaWebHttpLivenessTick()`** in **`spaWebServerLoop()`** — loopback **GET `/api/version`** when Wi‑Fi is up; **`ESP.restart()`** after consecutive probe failures (`HTTP liveness watchdog`; optional **`HTTP_LIVENESS_*`** in `config.h`). || TCP bridge (LAN clients) | [`lib/bridge/bridge.cpp`](lib/bridge/bridge.cpp) | Port **4257**; forwards to RS485 via [`cacheRead.cpp`](lib/spaMessage/cacheRead.cpp) / `sendMessageToSpa`. |
...
- **PRs:** Prefer a feature branch + PR into **`ESP32`** for substantive firmware/protocol/MQTT/web or release-bound changes. Wait for [`.github/workflows/build.yml`](.github/workflows/build.yml) (**`M5AtomLite-tub`**, **`ESP32ota`**, **`ESP32usb`** compile jobs; ruleset merge gate is **`M5AtomLite-tub`** + **`ESP32ota`** only — see [FORK.md](FORK.md); CI uses `config-example.h` only) and **automatic Codex** (on PR open). Do not comment `@codex review` on open by default; use it only if auto missed, after substantive follow-ups, or for a focused review. Direct push is OK for tiny docs/typos or when the maintainer chooses it. Remind; do not open PRs unless asked. See [FORK.md](FORK.md).
- **Testing:** Prefer **`pio run -e M5AtomLite-tub`** (and **`ESP32ota`** / **`ESP32usb`** when changing shared **`ESP32tub`** build flags) locally before asking for a PR; CI mirrors those compile checks.
