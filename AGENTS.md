...
| **Home Assistant MQTT Discovery** | [`lib/mqttModule/haMqttDiscovery.cpp`](lib/mqttModule/haMqttDiscovery.cpp) | After connect: **minimal** discovery (core status only). **Equipment** discovery expands after spa **config** / **information** frames (same rules as web `/status`); retracts stale optional entities for the session. **`availability_topic`** = `Spa/<gateway>/node/state`. See [HA MQTT Discovery](https://www.home-assistant.io/integrations/mqtt/#mqtt-discovery). |
