...
class AsyncAbstractResponse : public AsyncWebServerResponse {
  private:
    ...
    // Local patch (see _ack): wire bytes AsyncClient::write() refused (lwIP ERR_MEM under
    // pbuf/heap pressure), retried on later acks/polls before any new fill so no range of the
    // response is silently skipped and in-flight accounting stays honest.
    uint8_t* _carryBuf{nullptr};
    size_t _carryLen{0};
    size_t _carryOff{0};
    bool _carryTerminal{false};
