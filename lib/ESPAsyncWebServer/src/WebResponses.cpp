...
size_t AsyncAbstractResponse::_ack(AsyncWebServerRequest* request, size_t len, uint32_t time) {
  ...
  if (_state == RESPONSE_CONTENT) {
    ...
    // Local patch: flush bytes a previous write refused before filling anything new. The
    ...
    if (_carryBuf) {
      ...
      if (w) {
        ...
        --_in_flight_credit;
        _carryOff += w;
        if (_carryOff == _carryLen) {
          free(_carryBuf);
          _carryBuf = nullptr;
          _carryLen = _carryOff = 0;
          if (_carryTerminal) {
            _state = RESPONSE_WAIT_ACK;
          }
        }
    ...
    const bool terminal =
        (_chunked && readLen == 0) || (!_sendContentLength && outLen == 0) || (!_chunked && _sentLength == _contentLength);

    if (written < outLen) {
      // Keep the exact unsent wire bytes; the carry block above resends them on later
      // acks/polls before any new fill. buf ownership moves to the carry.
      _carryBuf = buf;
      _carryOff = written;
      _carryLen = outLen;
      _carryTerminal = terminal;
      return written;
    }
